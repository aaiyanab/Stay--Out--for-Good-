  ...
  return (
    <footer className="bg-navy-dark text-white/50 pt-16 pb-9 px-[8%]">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12 mb-12">
        {/* Brand */}
        <div>
          <Link href="/" className="block font-serif text-[1.3rem] font-bold text-white mb-3 no-underline">
            Stay Out <span className="text-gold-light">For Good</span>
          </Link>
          <p className="text-sm leading-relaxed text-white/50 max-w-[260px] mb-5">
            A national re-entry ministry by Leland Family Ministries, serving returning citizens, churches, and correctional ministry partners.
          </p>
        ...
        <div>
          ...
          <ul className="list-none m-0 p-0 flex flex-col gap-2">
            {FOOTER_LINKS.whoWeServe.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="text-white/50 text-sm hover:text-gold-light transition-colors no-underline">
                  {l.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="list-none m-0 p-0 flex flex-col gap-2">
            {FOOTER_LINKS.getInvolved.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="text-white/50 text-sm hover:text-gold-light transition-colors no-underline">
                  {l.label}
                </Link>
